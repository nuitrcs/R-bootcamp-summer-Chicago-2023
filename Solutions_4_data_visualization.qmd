...
penguin_plot  <-  ggplot(data = penguins , aes(x = flipper_length_mm, y = body_mass_g)) + 
  geom_point()
...
```{r}
penguin_plot + geom_smooth(method="lm") + coord_cartesian(xlim=c(180, 220))
```
...
```{r}
penguin_plot + labs(x = "Flipper length (mm)", y = "Body mass (g)", title = "Penguins Data")
```
...
```{r}
p1 <- ggplot(data = chic, aes(x=date, y=temp)) + geom_point(col="firebrick")
p1
```
...
```{r}
p2 <- p1 + labs(x = "Year", y = "Temperature", title = "Temperature in Chicago")
p2
```
...
```{r}
p3 <- p2 + theme(plot.title = element_text(color="darkgrey", face="bold"))
p3
```
...
p5 <- p4 + coord_cartesian(ylim = c(-10, 100))
p5
```
...
```{r}
# special function if axis is a date range
p6 <- p5 + scale_x_date(limits = as.Date(c("1995-01-01", "2005-01-01")))
p6
```
...
```{r}
p7 <- ggplot(data = chic, aes(x=date, y=temp, group=factor(season))) + 
  geom_point(aes(col=factor(season))) + # color points by season
  labs(x = "Year", y = "Temperature", title = "Temperature in Chicago")
p7
```
...
```{r}
p8 <- p7 + theme(legend.title = element_blank())
p8
```
...
```{r}
p11 <- p10 + facet_wrap(~year, ncol = 4)
p11
# facet_wrap and facet_grid are quite similar, except facet_wrap will only create plots when the combination of variables specified has some observations, while facet_grid will plpot and empty plot for such combinations.
```
...
```{r}
p12 <- p10 + facet_wrap(~year, ncol=2)
p12
```
...
```{r}
p13 <- p10 + facet_wrap(~year, ncol = 4) + geom_smooth(method="loess", col="black", fill="black", alpha=0.3)  # add geom_smooth
p13
```
...
p14 <- p10 + 
  ...
  geom_smooth(method="loess", col="black", fill="black", alpha=0.3)
p14
```
...
```{r}
p15 <- p10 + facet_wrap(~year + season , scales = "free") +
  geom_smooth(method="loess", col="black", fill="black", alpha=0.3) # with smoothing line added (optional)
p15
```
...
new_plot <- p10 + facet_wrap(~season , scales = "free")

p16 <- grid.arrange(p1, new_plot, ncol=2)
p16
```
...
p17a <- ggplot(data = chic, aes(x=date, y=temp, group=factor(season))) + 
  geom_point(aes(col=factor(season))) + # color points by season
  labs(x = "Year", y = "Temperature", title = "Temperature in Chicago") + theme_classic2() +
  scale_color_manual(values=c("gold", "greenyellow", "pink", "deepskyblue"))
p17a

# color names - https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf

# use pre-built color scales
p17 <- ggplot(data = chic, aes(x=date, y=temp, group=factor(season))) + 
  geom_point(aes(col=factor(season))) + # color points by season
  labs(x = "Year", y = "Temperature", title = "Temperature in Chicago") + theme_classic2() +
...
```{r}
p18 <- ggplot(data=chic, aes(x=season, y=o3)) +
  geom_boxplot() +
  coord_flip() # to flip the coordinates on the plot
p18
```
...
```{r}
p19 <- ggplot(data=chic, aes(x=season, y=o3, group=season)) +
  geom_violin(trim = FALSE, aes(fill=season)) + # `fill` colors inside the violins, `color` colors the outline of the violin shape
  theme_classic2()
p19
```
...
```{r}
p20 <- p19 + stat_summary(fun.data="mean_sdl", geom="pointrange", color="black")
p20
```
...
```{r}
p21 <- ggplot(data=chic, aes(x=date, y=temp)) + geom_point(alpha=0.5) + theme_classic2() +
  geom_smooth(method="loess", span=0.2) # set the span lower to get a more locally-fitted trendline
p21
```
...
```{r}
p22_high_span <- ggplot(data=chic, aes(x=date, y=temp)) + geom_point(alpha=0.5) + theme_classic2() +
  geom_smooth(method="loess", span=0.5)
p22_high_span

p22_low_span <- ggplot(data=chic, aes(x=date, y=temp)) + geom_point(alpha=0.5) + theme_classic2() +
...
p23 <- ggdensity(chic, x="o3", color="season", fill="season", alpha=0.5)
p23
```
...
```{r}
p24 <- ggdensity(chic, x="o3", color="season", fill="season", alpha=0.5, add="mean")
p24
```
...
```{r}
ggsave("my_saved_plot.pdf", p24, width=6, height=4, device="pdf", useDingbats=FALSE)
```
