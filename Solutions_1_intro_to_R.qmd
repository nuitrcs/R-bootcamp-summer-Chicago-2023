...
```{r}
setwd("R-bootcamp-summer-Chicago-2023")
```
...
Check that you are currently working within the `R-bootcamp-summer-Chicago-2023` Project. If not, open the relevant .RProj file in a new RStudio session.
...
100 * nrow(evp[evp$subject_sex=="female" & evp$vehicle_make=="TOYT",]) / nrow(evp[evp$vehicle_make=="TOYT",])
...
```{r}
aggregate(count ~ year, data=phds, sum)
tapply(phds$count, phds$year, sum)
```
...
```{r}
aggregate(count ~ school, data = phds[phds$year=="2021-22",], sum)
```
...
```{r}
total_11 <- aggregate(count ~ school, data = phds[phds$year=="2010-11",], sum)
total_22 <- aggregate(count ~ school, data = phds[phds$year=="2021-22",], sum)

total_22$difference <- total_22$count - total_11$count
total_22
```
