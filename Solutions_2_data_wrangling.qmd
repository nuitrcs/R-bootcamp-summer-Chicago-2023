...
```{r}
state.x77[seq(1, nrow(state.x77), 5),]
```
...
```{r}
evp$vehicle_make |> unique()
evp$vehicle_make |> unique() |> sort()
```
...
```{r}
as.logical(0)                   # Should be FALSE
as.logical(1)                   # Should be TRUE
as.logical(500)                 # Should be TRUE

as.character(c(2,3,4))          # "2" "3" "4"

class(is.logical(0)).           # "logical"
```
